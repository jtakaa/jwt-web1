...
const BemGuarda = (request, response) => {
  const token = request.headers["authorization"];
  ...
  try {
    ...
    response.json({ message: "VOCÊ DESCOBRIU, VAI CORINTHIANS!!!!"});
  } catch (error) {
    return response.status(403).json({ message: "Token inválido ou expirado" });
  }
};
...
const teste = (request, response) => {
  ...
  try {
    ...
    response.json({ message: "CLUB ATHLETICO PARANAENSE, NAO VAI CAIR."});
  } catch (error) {
    return response.status(403).json({ message: "Token inválido ou expirado" });
  }
};

module.exports = { login, protectedContent, BemGuarda, teste};
